...
describe("FieldDescription", () => {
  it("children을 렌더링한다", () => {
    ...
    expect(screen.getByText("이메일은 로그인에 사용됩니다.")).toBeInTheDocument();
  });

  it("id를 p element에 전달한다", () => {
    render(
      <FieldDescription id="email-description">이메일은 로그인에 사용됩니다.</FieldDescription>,
    );
  ...
  it("disabled 상태를 data attribute로 노출한다", () => {
    ...
    expect(screen.getByText("비활성 설명")).toHaveAttribute("data-disabled", "true");
  });
