...
describe("Admin ImageFrame", () => {
  ...
  it("src가 없으면 fallback을 렌더링한다", () => {
    render(<ImageFrame data-testid="image-frame" fallback={<span>이미지가 없습니다</span>} />);
  ...
  it("ratio, fit, fullWidth primitive prop을 전달한다", () => {
    ...
    expect(imageFrame).toHaveAttribute("data-ratio", "video");
  ...
  it("variant를 적용한다", () => {
    ...
    expect(screen.getByTestId("image-frame")).toHaveAttribute("data-variant", "surface");
  });
