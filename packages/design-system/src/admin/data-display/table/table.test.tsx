...
describe("Admin Table", () => {
  ...
  it("admin table container 스타일을 적용한다", () => {
    ...
    expect(tableContainer).toHaveClass("shadow-none");
  });
  ...
  it("admin head와 cell 스타일을 적용한다", () => {
    ...
    expect(screen.getByRole("columnheader", { name: "이름" })).toHaveClass("h-9");
    expect(screen.getByRole("columnheader", { name: "이름" })).toHaveClass("uppercase");
    expect(screen.getByRole("cell", { name: "MARSHOT" })).toHaveClass("h-10");
    expect(screen.getByRole("cell", { name: "MARSHOT" })).toHaveClass("py-2");
  });
  ...
  it("ref를 전달한다", () => {
    ...
    render(
      <Table ref={tableRef}>
        ...
        <TableHeader ref={headerRef} data-testid="header">
          <TableRow>
