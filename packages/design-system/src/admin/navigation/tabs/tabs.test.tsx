...
describe("Admin Tabs", () => {
  ...
  it("TabsList 기본 size는 md이고 fullWidth는 false이다", () => {
    ...
    const list = screen.getByRole("tablist", { name: "관리자 탭" });

    expect(list).toHaveAttribute("data-size", "md");
    expect(list).toHaveAttribute("data-full-width", "false");
    expect(list).toHaveClass("bg-muted/80", "border", "border-border");
  });

  it("TabsList size와 fullWidth를 지정할 수 있다", () => {
    render(
      <Tabs defaultValue="overview">
        <TabsList aria-label="관리자 탭" size="lg" fullWidth>
          <TabsTrigger value="overview">개요</TabsTrigger>
        </TabsList>
      </Tabs>,
    );
  ...
  it("className을 병합한다", () => {
    render(
      <Tabs defaultValue="overview" className="custom-tabs">
        <TabsList aria-label="관리자 탭" className="custom-list">
    ...
    expect(screen.getByRole("tablist", { name: "관리자 탭" })).toHaveClass("custom-list");
