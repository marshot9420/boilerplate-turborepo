...
describe("Admin Tooltip", () => {
  ...
  it("trigger에 hover하면 tooltip을 표시한다", async () => {
    ...
    render(
      <TooltipProvider delayDuration={0}>
        <Tooltip>
          ...
          <TooltipContent data-testid="content">관리자 도움말</TooltipContent>
        </Tooltip>
      </TooltipProvider>,
    );
    ...
    expect(await screen.findByTestId("content")).toHaveTextContent("관리자 도움말");
    expect(await screen.findByRole("tooltip")).toHaveTextContent("관리자 도움말");
  });
  ...
  it("content 기본 size는 md이다", async () => {
    ...
    render(
      <TooltipProvider delayDuration={0}>
        <Tooltip>
          ...
          <TooltipContent data-testid="content">관리자 정보</TooltipContent>
        </Tooltip>
      </TooltipProvider>,
    );
    ...
    expect(content).toHaveClass("max-w-64", "bg-foreground/95", "text-background", "shadow-lg");
  });
  ...
  it("TooltipArrow를 렌더링한다", async () => {
    ...
    expect(await screen.findByTestId("content")).toBeInTheDocument();
    expect(screen.getByTestId("arrow")).toHaveClass("fill-foreground/95");
  });
  ...
  it("className을 병합한다", async () => {
    ...
    expect(await screen.findByTestId("content")).toHaveClass("bg-foreground/95", "custom-tooltip");
  });
});
