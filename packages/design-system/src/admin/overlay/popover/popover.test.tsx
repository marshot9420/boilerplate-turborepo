...
describe("Admin Popover", () => {
  it("trigger를 클릭하면 popover를 연다", async () => {
    ...
    render(
      <Popover>
        <PopoverTrigger>필터 열기</PopoverTrigger>
        <PopoverContent data-testid="content">관리자 필터 콘텐츠</PopoverContent>
      </Popover>,
    );
    ...
    expect(screen.getByText("관리자 필터 콘텐츠")).toBeInTheDocument();
  });
  ...
  it("content 기본 size는 md이다", async () => {
    ...
    expect(content).toHaveClass("w-72", "bg-surface/95", "shadow-xl");
  });
  ...
  it("className을 병합한다", async () => {
    ...
    expect(screen.getByTestId("content")).toHaveClass("bg-surface/95", "custom-popover");
  });
});
